"""Gradient Boosted Regression Trees, with two main modifications done:
    1. Building models in leaves, and not predicting the regular average value (SVR in leaves)
    2. Constraints learning as part of the GBT being built
...
from ..feature_selection.from_model import SelectFromModel
...
from  fixes import expit, bincount
...
class ConstraintLoss(RegressionLossFunction):
    ...
    It inherts  from the regression loss function since they both work in a very similar way.
    Terminal regions need not to be updated for least squares also in this case. """
    ...
    def __call__(self, y, pred, sample_weight=None):
        ...
        if sample_weight is None:
            ...
            lse_part = (1.0 / sample_weight.sum() *
                        np.sum(sample_weight * ((y - pred.ravel()) ** 2.0)))
            constraints_part = (1.0 / sample_weight.sum() *
                                np.sum(sample_weight *
                                       constraints_with_pred.apply(lambda x:max(x['min_value'] - x['pred'], 0)**2 +
                                                                            max(x['pred'] - x['max_value'], 0)**2,
                                                                   axis=1)))

        return lse_part + self.constraints_gamma*constraints_part

    def negative_gradient(self, y, pred, **kargs):
        # the gradient is a composition here of two losses - the regular mse one and the constrints one - we'll
        # just sum both together
        lse_part = y - pred.ravel()
        constraints_part = (pred.ravel() < self.constraints_df['min_value'])*1 -\
                           (self.constraints_df['max_value'] < pred.ravel())*1 # maximum one of them will be true
        return lse_part + self.constraints_gamma*constraints_part
    ...
    def update_terminal_regions(self, tree, X, y, residual, y_pred,
        ...
        This part wasn't changed at all becuase of the constraints changes
        """
        # update predictions
...
class LeastSquaresError(RegressionLossFunction):
    ...
    #Our new function here (one out of a two) for cases of SVR model in leafs
    def my_update_terminal_regions(self, all_trees_models, tree, X, y, residual, y_pred,
                                sample_weight, sample_mask,
                                learning_rate=1.0, k=0, SVR_parmas = {'kernel': 'rbf', 'C': 1.0, 'epsilon': 0.1}):
        """Least squares needs to update terminal regions in this case, cause we want to build SVR model for each leaf
            In addition, it updates the prediction (as was done before we changed the fucntion)

            Parameters
            ----------
            All the same one as was sent in the original 'update_terminal_regions' function, besides the following ones:

            all_trees_models : list, shape=#of trees built in the model
                Holds all the models built so far for each tree. We will have to add to this object the current models which will be built

            SVR_parmas: dictionary
                Holds the SVR parameters to be used when building an SVR model
        """

        #this will hold all the models we are building for this specific tree -
        # this list will be added at last to 'all_trees_models'
        cur_tree_models = []
        observation_location = tree.apply(X)
        cur_prediction = np.zeros(observation_location.size)
        #going over each node in the tree
        for i in range(0,tree.capacity):
            #case the current node is not a leaf - we'll still hold a 'model' there,
            # it will be -1 model (kind of a NULL model)
            if tree.children_left[i]!=-1:
                cur_tree_models.append(-1)
            # case the current node is a leaf - we'll build a model and will save it as an SVR model
            else:
                relevant_idx = np.where(observation_location==i)[0]
                #getting the relevant X,y values for the current leaf
                cur_X = X[(relevant_idx),:]
                cur_Y = residual[relevant_idx]
                #building a model with the input parameters for SVR and running a prediction
                clf = SVR(**(SVR_parmas))
                cur_model = clf.fit(X = cur_X, y = cur_Y)
                cur_tree_models.append(cur_model)
                cur_prediction[relevant_idx] = cur_model.predict(cur_X)

        #adding the list we created with all models to the big 'cur_tree_models' list
        all_trees_models.append(cur_tree_models)
        #updating the prediction
        y_pred[:,k]+=learning_rate * cur_prediction.ravel()
...
class BaseGradientBoosting(six.with_metaclass(ABCMeta, BaseEnsemble, SelectFromModel)):
    ...
    def __init__(self, constraint_obj,
        ...
        self.constraints_gamma = constraint_obj.constraints_gamma
        self.constraints_eta = constraint_obj.constraints_eta
        ...
        self.all_trees_models = []

    def add_model(self,model):
        self.all_trees_models.append(model)
    ...
    def _check_params(self):
        ...
        if self.loss == 'constraints':
            self.loss_.constraints_df = self.constraints_df
            self.loss_.constraints_gamma = self.constraints_gamma
    ...
    def fit(self, X, y, sample_weight=None, monitor=None, weights_based_constraints_sol = False):
        ...
        self : object
            Returns self.
        """
        self.constraints_weights = np.ones(X.shape[0], dtype=np.float32)
        ...
        n_stages = self._fit_stages(X = X, y = y, y_pred = y_pred, sample_weight = sample_weight,
                                    random_state=random_state, weights_based_constraints_sol=weights_based_constraints_sol,
                                    begin_at_stage=begin_at_stage, monitor=monitor, X_idx_sorted=X_idx_sorted)
    ...
    def _fit_stages(self, X, y, y_pred, sample_weight, random_state, weights_based_constraints_sol,
        ...
        for i in range(begin_at_stage, self.n_estimators):
            ...
            if weights_based_constraints_sol:
                ...
                for index, row in self.constraints_df.iterrows():
                    ...
                    if still_alive and (y_pred[index] > row['max_value'] or y_pred[index] < row['min_value']) and self.constraints_weights[index] < 1E+20:
                        self.constraints_weights[index] = self.constraints_weights[index]*(1 + self.constraints_eta)
            # subsampling
            if do_oob:
    ...
    def my_decision_function(self, X):
        """Compute the decision function of ``X``. In this specific function, we are using the SVR models of the leafs in order
        to provide a prediction
        The prediction should be as follow(case of 3 trees): avg() + tree0_pred*learning_rate + tree1_pred*learning_rate + tree2_pred*learning_rate
        ...
        final_prediction : array
                            The prediciton for each observation, based on the relevatn model should be used (SVR model)
        """
        X = check_array(X, dtype=DTYPE, order="C")
        avg_score = self.init_.mean


        final_prediction = np.empty(X.shape[0])
        final_prediction.fill(avg_score)
        #go over each estimator (<-> tree)
        for i in range(0,self.estimators_.size):
            cur_tree = self.estimators_[i][0].tree_
