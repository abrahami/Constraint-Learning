...
for j in range(config_df.shape[0]):
    ...
    for cur_fold in range(cv_folds_amount):
        ...
        constraint_reg_obj = ConstraintRegressor(cv_params={'percentile_threshold': cur_config['percentile_threshold'],
                                                 ...
                                                 cv_folds_size=cv_folds_amount)
        data = constraint_reg_obj.load_dataset(data_path=data_loc, which_fold=cur_fold)
        ...
        if save_evaluation_measures:
            ...
            if not file_exists:
                writer.writerow(['time', 'index', 'Dataset', 'percentile_threshold', 'constraint_interval_size',
                                  ...
                                  'constraints_eta', 'random_state', 'constraint_gamma', 'early_stopping', 'cur_fold',
                                 'Algo_type', 'train / test', 'Duration(sec)', 'Constraints_count',
                                 'CMSE', 'MSE', 'CER', 'R_square'])
        ...
        if run_option_b:
            ...
            # a list with a single value or just an integer/float (and not a list at all)
                ...
                if save_evaluation_measures:
                    cur_train_full_log = copy.copy(config_details)
                    cur_train_full_log.extend((cur_fold, "Option B - global weight to constrainted", "train", duration,
                                               cur_train_eval['n_constraints'], cur_train_eval['CMSE'],
                                               cur_train_eval['MSE'], cur_train_eval['CER'], cur_train_eval["R_square"]))
                    writer.writerow(cur_train_full_log)
                    cur_test_full_log = copy.copy(config_details)
                    cur_test_full_log.extend((cur_fold, "Option B - global weight to constrainted", "test", duration,
        ...
        if run_option_c:
            if constraint_reg_obj.constraint_interval_size == 0:
                ...
                if save_evaluation_measures:
                    cur_train_full_log = copy.copy(config_details)
                    cur_train_full_log.extend((cur_fold, "Option C - smart weight to constrainted", "train", duration,
                                               cur_train_eval['n_constraints'], cur_train_eval['CMSE'],
                                               cur_train_eval['MSE'], cur_train_eval['CER'], cur_train_eval["R_square"]))
                    writer.writerow(cur_train_full_log)

                    cur_test_full_log = copy.copy(config_details)
                    cur_test_full_log.extend((cur_fold, "Option C - smart weight to constrainted", "test", duration,
                                              cur_test_eval['n_constraints'], cur_test_eval['CMSE'], cur_test_eval['MSE'],
                                              cur_test_eval['CER'], cur_train_eval["R_square"]))
                    writer.writerow(cur_test_full_log)
                # case we want to save prediction of each observation
        ...
        if run_option_d:
            ...
            # single run without running over few etas
                ...
                if save_evaluation_measures:
                    cur_train_full_log = copy.copy(config_details)
                    cur_train_full_log.extend((cur_fold, "Option D - our 1st algo - dynamic weight", "train", duration,
                                               cur_train_eval['n_constraints'], cur_train_eval['CMSE'],
                                               cur_train_eval['MSE'], cur_train_eval['CER'], cur_train_eval["R_square"]))
                    writer.writerow(cur_train_full_log)

                    cur_test_full_log = copy.copy(config_details)
                    cur_test_full_log.extend((cur_fold, "Option D - our 1st algo - dynamic weight", "test", duration,
                                              cur_test_eval['n_constraints'], cur_test_eval['CMSE'], cur_test_eval['MSE'],
                                              cur_test_eval['CER'], cur_train_eval["R_square"]))
                    writer.writerow(cur_test_full_log)
                # case we want to save prediction of each observation
        ...
        if run_option_e:
            ...
            if type(constraint_reg_obj.constraints_gamma) is list:
                ...
                if save_evaluation_measures:
                    cur_train_full_log = copy.copy(config_details)
                    cur_train_full_log.extend((cur_fold, "Option E-our 2nd algo - new loss function", "train", duration,
                                               cur_train_eval['n_constraints'], cur_train_eval['CMSE'],
                                               cur_train_eval['MSE'], cur_train_eval['CER'], cur_train_eval["R_square"]))
                    writer.writerow(cur_train_full_log)
                    cur_test_full_log = copy.copy(config_details)
                    cur_test_full_log.extend((cur_fold, "Option E-our 2nd algo - new loss function", "test", duration,
                                              cur_test_eval['n_constraints'], cur_test_eval['CMSE'], cur_test_eval['MSE'],
                                              cur_test_eval['CER'], cur_train_eval["R_square"]))
                    writer.writerow(cur_test_full_log)
                # case we want to save prediction of each observation
